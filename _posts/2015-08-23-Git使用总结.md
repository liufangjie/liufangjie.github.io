...
2. 撤销add操作
[Git-基础-撤消操作](http://git-scm.com/book/zh/v1/Git-%E5%9F%BA%E7%A1%80-%E6%92%A4%E6%B6%88%E6%93%8D%E4%BD%9C)
...
1git clone git@192.168.0.102:gitolite-admin1
...
4. 开发人员尽量按照下述步骤push代码
 ...
 11. 推送内容到远端`git push`
 如果此时又有人push新内容到dev分支，push时git会提示需要重新拉取最新内容,此时需要将dev分支reset到merge之前，进行pull，如果直接pull会有冲突
 12. push时提示需要pull的情况下，先reset dev分支到merge之前的版本
 * `git reflog`找到merge之前的commit sha1值
 * `git reset --hard sha1`
 * `git pull`拉取最新内容
