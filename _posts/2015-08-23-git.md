...
在centos 7 minimal上按照[Pro Git](https://git-scm.com/book/zh/v2/%25E8%25B5%25B7%25E6%25AD%25A5-%25E5%25AE%2589%25E8%25A3%2585-Git)上的步骤安装会失败，有其他的工具需要安装，以下记录

安装的时候需要的工具

`yum groupinstall development -y`

`yum install perl-devel perl-CPAN libxml2 libxml2-devel -y`

libxml2是docbook2x的依赖

docbook2x-text安装步骤：

[docbook2x](http://docbook2x.sourceforge.net/)

[docbook2x install](http://docbook2x.sourceforge.net/latest/doc/install.html)

1. 下载最新版[0.8.8](https://sourceforge.net/projects/docbook2x/files/docbook2x/0.8.8/)

2. 解压 `tar -xzf docbook2X-0.8.8.tar.gz`

3. 执行 `./configure` 然后 `make,make install`

docbook2x安装完成后需要执行如下命令

`ln -s /usr/local/bin/docbook2texi /usr/local/bin/docbook2x-texi`

可能是docbook2x新版本将bin命令的名字修改为db2x_docbook2texi了，所以需要用ln建立个软链接

由于doc无法编译成功所以`make all doc info` 改为 `make all`

之后执行安装还是会提示install-info出错，把install info去掉
